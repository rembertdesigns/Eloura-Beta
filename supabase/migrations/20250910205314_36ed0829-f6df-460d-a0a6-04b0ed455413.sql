-- Fix complex conversation-related RLS policies for better performance
-- Replace direct auth.uid() calls in complex policies with subqueries
...
FOR SELECT
TO authenticated
USING ((user_id = (select auth.uid())) OR is_conversation_participant(conversation_id, (select auth.uid())));
...
FOR INSERT
TO authenticated
WITH CHECK (((select auth.uid()) = sender_id) AND (conversation_id IN ( 
  SELECT conversation_participants.conversation_id
  FROM conversation_participants
  WHERE (conversation_participants.user_id = (select auth.uid()))
)));
