-- Security Fix: Reduce PII exposure in user_onboarding table
-- Move sensitive data to profiles table and add encryption functions

-- 1. First, add missing columns to profiles table for secure PII storage
...
-- 2. Create encryption function for sensitive data (using pgcrypto if available)
...
AS $$
BEGIN
  -- Simple obfuscation for now - can be enhanced with proper encryption
...
-- Update profiles with data from user_onboarding where it exists
...
-- 5. Add audit function for sensitive data access
...
-- 6. Add trigger for PII access logging on profiles
CREATE TRIGGER audit_profiles_pii_access
  AFTER SELECT ON public.profiles
  FOR EACH ROW
  WHEN (current_setting('log_pii_access', true) = 'true')
  EXECUTE FUNCTION public.audit_pii_access('address,phone,emergency_contact_name,emergency_contact_phone');

-- 7. Create view for safe profile access (without full PII exposure)
